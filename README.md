...
# 心率：有值则保留一位小数，无值（未戴设备）传 None，便于前端折线断点与 JSON 序列化
heart_rates = [
...
2. **高颜值 Dashboard 布局**
   ...
   - 上方一行：4 张统计卡片，展示：
     ...
     - 平均单次距离
     - 最长单次距离
   - 中间：占据主要视窗的**双 Y 轴主图**「跑步距离与平均心率」（见下方 4.3 节）；
...
看板主图采用**双 Y 轴（Dual Y-Axis）**，在同一张图中同时展示「跑步距离」与「平均心率」，便于对比单次跑步的强度与量级。

- **左侧 Y 轴**：距离 (km)，保留原有样式（虚线网格、自适应最小值），对应蓝色折线；折线带渐变面积阴影，并保留「最远」markPoint 与「均值」markLine。
- **右侧 Y 轴**：心率 (bpm)，`position: 'right'`，`scale: true` 自动缩放；**隐藏右侧网格线**（`splitLine: { show: false }`），避免与左侧网格重叠，保持界面整洁。
- **距离曲线**：蓝色（`#5b8dee`），平滑曲线 + 面积填充，`yAxisIndex: 0`。
- **心率曲线**：红/粉色系（`#ff4d4f`），平滑曲线、**虚线**（`lineStyle: { type: 'dashed' }`）、**无面积阴影**，与距离曲线视觉区分；`yAxisIndex: 1`。若某条记录无心率数据（后端传 `null`），该点在折线上会断点显示，不连成错误数值。
- **Tooltip 交互**：鼠标悬浮时，提示框**同时**展示该日期的「距离 (km)」与「平均心率 (bpm)」；无心率时显示「无数据」，保证信息完整且不误导。

后端对心率的处理：`average_heart_rate` 有值时取 `round(..., 1)` 传入列表，为 `null` 时传入 Python `None`，经 `json.dumps` 序列化为 JSON 的 `null`，前端 ECharts 对 `null` 会自动断线，不会报错。
